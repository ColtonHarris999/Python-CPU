...
| CPython / PyCPython `codegen.py` + `assemble.py` | PSF-2.0 | `vendor/pycpython` (reference only) | CPython 3.14.7 | T1–T4 instruction shapes, jump offsets, stacksize walk, 6-bit exception-table varints. | Device `codegen.py` is a recursive SoA visit plus assembler calling `_bi_code_*`. No `CACHE`, no constant folding. Nested `def` assembles into the parent `co_consts` then `MAKE_FUNCTION`. T4 emits `RAISE_VARARGS` / try handlers / `LIST_APPEND`/`SET_ADD`/`MAP_ADD` / string `BINARY_SLICE`; exception tables are INT-tuple varints. Host `encode_exception_table` (W-3) uses the same varint layout. |
