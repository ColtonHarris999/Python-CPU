...
| CPython / PyCPython `symtable.py` | PSF-2.0 | `vendor/pycpython` (reference only) | CPython 3.14.7 | Module + function scope, `global`, closure detection. | Device `symtab.py` is an original iterative walk of the SoA AST. Closures emit `MAKE_CELL` / `LOAD_DEREF` / `COPY_FREE_VARS`; nested enclosing loads are freevars. |
| CPython / PyCPython `codegen.py` + `assemble.py` | PSF-2.0 | `vendor/pycpython` (reference only) | CPython 3.14.7 | T1–T4 instruction shapes, jump offsets, stacksize walk, 6-bit exception-table varints. | Device `codegen.py` is a recursive SoA visit plus assembler calling `_bi_code_*`. No `CACHE`. §11.3 folds int ALU / str Add in `_pyc_codegen_main`. Nested `def` assembles into the parent `co_consts` then `MAKE_FUNCTION`. T4 emits `RAISE_VARARGS` / try handlers / `LIST_APPEND`/`SET_ADD`/`MAP_ADD` / string `BINARY_SLICE`; exception tables are INT-tuple varints. Host `encode_exception_table` (W-3) uses the same varint layout. |
