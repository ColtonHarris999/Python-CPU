...
| CPython / PyCPython `codegen.py` + `assemble.py` | PSF-2.0 | `vendor/pycpython` (reference only) | CPython 3.14.7 | T1 instruction shapes, jump offsets, stacksize walk, 6-bit exception-table varints. | Device `codegen.py` is a recursive SoA visit plus assembler calling `_bi_code_*`. No `CACHE`, no constant folding, no nested-`def` emit in T1. Host `encode_exception_table` (W-3) uses the same varint layout. |
