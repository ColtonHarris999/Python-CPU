...
| **P7** | Exception types with messages. Wave A types + construction/unwind are on `main`; `e.args` read done (F4); firmware `raise <int>` → F1 done. Cross-frame `try`/`except` works (exceptions T3). |
