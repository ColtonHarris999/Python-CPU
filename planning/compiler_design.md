...
1. **A compiler written in PyCore-subset Python**, derived from the
...
2. **A boot image that seeds it**: the image builder compiles that tree with
...
3. **Four new hardware builtins** (`_bi_code_alloc`, `_bi_code_blit`,
...
Everything else is a rewrite of the compiler, not a change to the CPU. That
split is deliberate and is argued in §3 and §6.
...
| Code writes already traverse xbar → L2 → RAM | `pycore_mem_xbar.sv:139-149` (imem write with `wstrb` hi/lo select), `pycore_ram.sv:253-255, 286-288` (`code_arr` merge-strobe write) | The write path is 90% built (§6.1). |
...
| `pycore/docs/memory_hierarchy.md` invalidation matrix | heap release is absent from the CODC/GIC flush rows | `_bi_heap_release` can recycle an address a CODC/GIC entry is keyed on. Latent today (no runtime code objects); reachable after this work (§6.3). |
...
**C1 — 32 locals per function, enforced by a fatal trap.**
`pycore_call_fsm.svh:2392`: `local_slots > 16'd32` → `call_filter_trap_r`.
Every function in the compiler, and every function the compiler emits, must
have `nlocals (params included) ≤ 32`. The compiler must reject user code that
exceeds it with a `SyntaxError`, not emit it (A4).
...
> This is the single most important constraint. A recursive-descent compiler
> whose depth scales with source nesting will exhaust the RF on ordinary
> programs and halt. The design answer is §5.2: **every pass is iterative**.
...
| any function with > 32 locals | split it | **C1**, fatal trap |
| recursion whose depth follows source nesting | explicit stacks (§5.2) | **C2**, fatal trap |
...
for every function F in pycore_firmware/compiler/:
    assert F.__code__.co_nlocals <= 32                # C1
    assert F.__code__.co_stacksize <= 24              # C2 headroom
...
**Rule 2 — no pass may recurse on source structure.** Concretely:

| Pass | Conventional shape | Required shape here |
| --- | --- | --- |
| Expression parse | recursive descent, one frame per precedence level per nesting level | **explicit operand + operator stacks** (precedence climbing / shunting-yard). One frame, any depth. `(`, `[`, `{`, and call-open push a bracket marker; the matching close reduces down to it. |
| Statement parse | recursive descent on suites | **one loop** over the token stream with an explicit block stack driven by `INDENT` / `DEDENT`. One frame, any depth. |
| Symbol table | recursive AST walk | explicit work stack of node ids |
| Codegen | `VISIT(expr)` recursion | explicit work stack of `(node_id, phase)` — push `ENTER`, re-push `EXIT` for post-order emission |
| Assembler | already a loop | loop |

The result is a stated, testable invariant:

> **Frame-depth invariant.** The firmware compiler's live call depth is a
> constant, independent of the source it compiles. `img_compile_deep_nesting`
> pins it (A3), and a `co_stacksize` / `nlocals` budget in the subset gate
> keeps every frame small.

This single decision removes the entire "Python is not LL(1)", "PEG blows the
frame stack", and "grow `RF_DEPTH`" problem from the plan. It is also why LL(1)
table generation is **not** needed: precedence climbing with explicit stacks
handles Python's expression grammar directly, and the statement grammar is
already `INDENT`-delimited and effectively LL(1) on the first token.
...
| `assemble.py` | 426 | **closest port** | Exception-table 6-bit varints, jump offsets, `co_stacksize`. Replace `build_code` / `marshal.loads` with `_bi_code_new`. Its varint writer is also the source for the host-side `encode_exception_table` (§6.4 W3). |
...
| D6 | `> 32` locals, `co_stacksize > 24`, or a closure is a `SyntaxError` | C1 / C2 surfaced at compile time instead of as a fatal trap. |
...
### 6.1 RTL — the code-memory write path (R-1 … R-4)
...
### 6.3 RTL — cache and inline-cache coherence (R-6)
...
| **W-7** | `pycore/tests/test_compiler_subset.py` | §5.1 gate. Write it **first** — it is step A in §9. |
| **W-8** | `make pycore-size-report` | Per-region slots / heap bytes vs ceilings; **fails the build** on overflow (A8). |

### 6.5 Documentation to update in the same commits
...
| `pycore/docs/memory_hierarchy.md` | invalidation matrix rows from §6.3 |
...
| Growing `RF_DEPTH` | The frame-depth invariant (§5.2 Rule 2) removes the pressure |
...
| Register file | 256 entries, 32 reserved | frame-depth invariant; per-frame `nlocals ≤ 32`, `stacksize ≤ 24` | C1, C2 |
| Frame stack | 32 KB / 1 024 descriptors | never the binding limit | `pycore_defs.svh:3100-3101` |
...
| # | Risk | Mitigation |
| --- | --- | --- |
| R1 | A pass accidentally recurses on source structure and blows the RF (C2) | `img_compile_deep_nesting` (A3) plus a subset-gate rule that no compiler function calls itself or a mutually-recursive peer |
...
Each step is done when a **test** is green. Steps T0, A, and B are the
prerequisites everything else depends on; do them in order. C–G are host-side
and can be developed in parallel with R-1…R-6.
...
| **A** | `test_compiler_subset.py` + `compat` helpers + empty `pycore_firmware/compiler/` | Host test is **red** on `xs[-1]`, `xs[1:]`, `class`, a 33-local function, a closure |
| **B** | R-1…R-6 + W-4 + W-5 (the whole write path and the four builtins) | `img_code_new_call`: blit `RESUME; LOAD_SMALL_INT 7; RETURN_VALUE`, `_bi_code_new`, call it, get 7. Plus `img_code_emit_then_call`, `img_code_alloc_oom_trap`, `img_code_write_floor_trap` |
| **C** | W-1, W-2, W-6: seed a two-function toy package into `_PYC_G` and call one from the other | `img_pyc_package_call` |
| **D** | `tables.py` + `lexer.py` | Host: token stream vs CPython `tokenize` on a small corpus. Device: `img_lexer_count` |
| **E** | `parser.py` T1 expressions (iterative) + SoA AST | Host: shape round-trip vs `ast.parse`. Device: `img_parser_tiny_expr`, `img_compile_deep_nesting` |
| **F** | `symtab.py` | Host locals-vs-globals corpus; closures raise cleanly |
| **G** | `codegen.py` T1 + `assemble.py` + W-3 | Host result differential vs CPython for T1 |
| **H** | Wire `compile.py` shim; ship it | **`img_compile_eval_expr` → 3** (A1) |
| **I** | T2 then T3 | `img_compile_exec_roundtrip` (A2) + host corpus |
| **J** | W-8 size report, doc sweep (§6.5), deviation table | `make all-tests` green; report within budget |
...
| `img_compile_reject_locals` | `SyntaxError` on a 33-local function (D6) |
