...
\begin{tikzpicture}[
  ...
  state/.style={
    ...
    inner sep=3pt, fill=pyfill, draw=pyborder
  },
  bootst/.style={state, fill=toolfill, draw=toolborder},
  trapst/.style={state, fill=exfill, draw=exborder},
  haltst/.style={state, fill=memfill, draw=memborder},
  arr/.style={-{Latex}, thick, color=arrowc},
  lab/.style={font=\scriptsize, text=black!75, align=center},
  note/.style={font=\scriptsize\itshape, text=black!60, align=left}
]
  % --- classic pipeline strip ---
  \node[bootst] (boot)  at (0, 4.6)   {\texttt{S\_BOOT}\\{\scriptsize(9)}};
  \node[state]  (fetch) at (0, 3.0)   {\texttt{S\_FETCH}\\{\scriptsize(0)}};
  \node[state]  (dec)   at (2.6, 3.0) {\texttt{S\_DECODE}\\{\scriptsize(1)}};
  \node[state]  (exec)  at (5.2, 3.0) {\texttt{S\_EXEC}\\{\scriptsize(2)}};
  \node[state]  (mem)   at (7.8, 3.0) {\texttt{S\_MEM}\\{\scriptsize(3)}};
  \node[state]  (wb)    at (10.4, 3.0){\texttt{S\_WB}\\{\scriptsize(4)}};

  % --- language / call arms ---
  \node[state] (cont)  at (5.2, 1.0) {\texttt{S\_CONTAINER}\\{\scriptsize(8)}};
  \node[state] (call)  at (8.2, 1.0) {\texttt{S\_CALL}\\{\scriptsize(6)}};
  \node[state] (ret)   at (10.8, 1.0){\texttt{S\_RETURN}\\{\scriptsize(7)}};

  % --- trap / halt ---
  \node[trapst] (tm)   at (5.2, -1.1){\texttt{S\_TRAP\_MARSHAL}\\{\scriptsize(10)}};
  \node[trapst] (tw)   at (8.4, -1.1){\texttt{S\_TRAP\_WAIT}\\{\scriptsize(11)}};
  \node[haltst] (hlt)  at (10.8, -1.1){\texttt{S\_HALT}\\{\scriptsize(5)}};
  ...
  \draw[arr] (wb.north) -- ++(0,0.55) -| node[lab,pos=0.2,above]
    {ordinary op} (fetch.north);

  % --- container bypass ---
  \draw[arr] (exec)  -- node[lab,right]{route\_container} (cont);
  \draw[arr] (cont.west) -- ++(-1.6,0) |- node[lab,pos=0.25,left]
    {CP\_DONE} (fetch.west);

  % --- call / return from WB ---
  \draw[arr] (wb) -- node[lab,right]{CALL*} (call);
  \draw[arr] (wb) -- node[lab,right,pos=0.4]{RETURN (depth$>$0)} (ret);
  \draw[arr] (call.east) -- ++(0.7,0) |- node[lab,pos=0.2,right]
    {CALL\_DONE} (fetch.east);
  \draw[arr] (ret.east)  -- ++(1.05,0) |- node[lab,pos=0.15,right]
    {RET\_DONE} (fetch.east);

  % --- container <-> call (protocol) ---
  \draw[arr] (cont) -- node[lab,above]{protocol CALL} (call);
  \draw[arr] (ret)  -- node[lab,above,sloped,pos=0.45]
    {resume container} (cont);

  % --- recoverable trap path ---
  \draw[arr] (cont) -- node[lab,right]{recoverable} (tm);
  \draw[arr] (call.south) |- node[lab,pos=0.25,below]{BUILTIN\_CALL} (tm);
  \draw[arr] (tm) -- node[lab,above]{req ready} (tw);
  \draw[arr] (tw.south) -- ++(0,-0.55) -| node[lab,pos=0.2,below]
    {COMPLETED / RETRY} (fetch.south);
  \draw[arr] (tw) -- node[lab,above]{FATAL} (hlt);

  % --- fatal freeze ---
  \draw[arr, densely dashed] (hlt) to[loop below] node[lab]{freeze} (hlt);
  \node[note, anchor=west] at (-1.8, -1.1)
    {any \texttt{pycore\_trap}\\$\Rightarrow$ \texttt{S\_HALT}};

\end{tikzpicture}%
}
\caption{PyCore top-level control FSM (\texttt{state\_r}).
The five-stage strip is the typed fast path.
\texttt{S\_CONTAINER} and \texttt{S\_CALL}/\texttt{S\_RETURN} bypass or follow
writeback for heap and frame work; recoverable traps exit through
\texttt{S\_TRAP\_MARSHAL}/\texttt{S\_TRAP\_WAIT} to ExCore.
Protocol iteration pauses \texttt{S\_CONTAINER}, runs \texttt{S\_CALL}, and
resumes via \texttt{S\_RETURN}.}
\label{fig:core-fsm}
\end{figure}
