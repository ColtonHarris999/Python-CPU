...
Figure~\ref{fig:block} is the \emph{target} physical SoC organization:
SRAMs, datapath units, language engines, interconnect buses, and the RV32
helper core.
It deliberately omits control-pipeline stages; those belong in the FSM figure
that follows.

\clearpage
\begin{figure}[p]
\centering
{\Large\bfseries Ideal PyCore + ExCore SoC}\\[0.35em]
{\small Physical blocks and buses}\\[0.75em]
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
  font=\small,
  >=Latex,
  x=1cm, y=1cm,
  blk/.style={
    draw, align=center, inner sep=4pt, thick,
    rounded corners=1.5pt, minimum height=0.85cm
  },
  ...
  bus/.style={Latex-Latex, line width=1.05pt, color=arrowc},
  arr/.style={-{Latex}, line width=0.9pt, color=arrowc},
  ...
  map/.style={
    ...
    inner xsep=3pt, inner ysep=2pt, minimum height=0.4cm, text width=3.7cm
  },
  grp/.style={draw, line width=1.15pt, rounded corners=3pt, inner sep=6pt},
  busbar/.style={line width=2.2pt, color=black!35}
]

% ===================== SHARED BUSES (horizontal bars) =====================
% Instruction bus / data bus / trap bus as physical interconnects
\draw[busbar] (-6.6, 8.55) -- (6.6, 8.55); % I-bus
\draw[busbar] (-6.6, 5.55) -- (6.6, 5.55); % D-bus
\draw[busbar] (-6.6, 2.85) -- (6.6, 2.85); % Trap / MMIO bus
\node[lab] at (-7.35, 8.55) {I-bus};
\node[lab] at (-7.35, 5.55) {D-bus};
\node[lab] at (-7.45, 2.85) {Trap bus};

% ===================== MEMORIES (top / bottom anchors) =====================
\node[mem, minimum width=2.7cm] (crom) at (-5.2, 10.0) {Code ROM\\{\scriptsize BIOS+runtime}};
\node[mem, minimum width=2.7cm] (cram) at (-2.2, 10.0) {Code RAM\\{\scriptsize compiled code}};
\node[mem, minimum width=2.7cm] (dmem) at (1.2, 10.0) {Shared DMEM\\{\scriptsize 128-bit SRAM}};
\node[mem, minimum width=2.7cm] (fimem) at (4.5, 10.0) {FW IMEM\\{\scriptsize private}};
\node[mem, minimum width=2.0cm] (spm) at (6.55, 10.0) {Scratch\\{\scriptsize SRAM}};

\draw[arr] (crom) -- (crom |- 0,8.55);
\draw[arr] (cram) -- (cram |- 0,8.55);
\draw[arr] (dmem) -- (dmem |- 0,8.55);
\draw[arr] (dmem) -- (dmem |- 0,5.55);
\draw[arr] (fimem) -- (fimem |- 0,8.55);
\draw[arr] (spm) -- (spm |- 0,5.55);

% ===================== PYCORE DATAPATH =====================
\node[py, minimum width=2.55cm] (core) at (-5.2, 7.15)
  {\textbf{Bytecode core}\\{\scriptsize PC $\cdot$ decode $\cdot$ seq}};
\node[mem, minimum width=2.55cm] (rf) at (-2.2, 7.15)
  {\textbf{Regfile}\\{\scriptsize 96$\times$132 tagged}};

\node[py, minimum width=1.35cm] (alu) at (-5.95, 6.2) {INT ALU};
\node[py, minimum width=1.35cm] (fpu) at (-4.4, 6.2) {FPU};
\node[py, minimum width=1.35cm] (mul) at (-2.85, 6.2) {MUL/DIV};
\node[py, minimum width=1.35cm] (cpx) at (-1.3, 6.2) {Complex};

\node[grp, draw=pyborder, inner sep=5pt,
  fit=(core)(rf)(alu)(fpu)(mul)(cpx),
  label={[ttl,text=pyborder]above:\textbf{PyCore datapath}}] (pyd) {};

\draw[arr] (core) -- (core |- 0,8.55);
\draw[arr] (core) -- (core |- 0,5.55);
\draw[bus] (core) -- (rf);
\draw[bus] (rf) -- (alu);
\draw[bus] (rf) -- (fpu);
\draw[bus] (rf) -- (mul);
...
% ===================== LANGUAGE ENGINES =====================
\node[py, minimum width=1.85cm] (fr) at (-5.7, 4.45)
  {Frame mgr\\{\scriptsize stack SRAM}};
\node[py, minimum width=1.85cm] (st) at (-3.55, 4.45)
  {String unit\\{\scriptsize str SRAM}};
\node[py, minimum width=1.85cm] (ct) at (-1.4, 4.45)
  {Container\\{\scriptsize eng.}};
\node[py, minimum width=1.85cm] (hs) at (-5.7, 3.5)
  {Hash / eq\\{\scriptsize unit}};
\node[py, minimum width=1.85cm] (cb) at (-3.55, 3.5)
  {CALL binder\\{\scriptsize HW}};
\node[py, minimum width=1.85cm] (xe) at (-1.4, 3.5)
  {Exc engine\\{\scriptsize HW}};
\node[py, minimum width=4.0cm] (ha) at (-3.55, 2.55)
  {Heap unit (bump + GC)};

\node[grp, draw=pyborder, inner sep=5pt,
  fit=(fr)(st)(ct)(hs)(cb)(xe)(ha),
  label={[ttl,text=pyborder]above:\textbf{Language engines}}] (pya) {};

\foreach \n in {fr,st,ct,hs,cb,xe,ha} {
  \draw[arr] (\n) -- (\n |- 0,5.55);
}

% ===================== INTERCONNECT BLOCKS =====================
\node[mem, minimum width=2.9cm] (grant) at (1.35, 7.15)
  {DMEM grant mux\\{\scriptsize 1 owner}};
\node[mem, minimum width=2.9cm] (mbox) at (1.35, 4.0)
  {Trap mailbox\\{\scriptsize HW FIFO}};
\node[io, minimum width=2.9cm] (uart) at (1.35, 1.7)
  {Console / I/O\\{\scriptsize TX port}};

\draw[arr] (grant) -- (grant |- 0,5.55);
\draw[arr] (grant) -- (dmem);
\draw[arr] (mbox) -- (mbox |- 0,2.85);
\draw[arr] (mbox) -- (mbox |- 0,5.55);
\draw[arr] (uart) -- (uart |- 0,2.85);

% Memory map panel
\node[font=\scriptsize\bfseries, text=memborder] at (1.35, -0.05) {DMEM map};
\node[map] (m0) at (1.35, -0.55) {0x03E0 boot / globals};
\node[map] (m1) at (1.35, -1.05) {0x0440 object heap};
\node[map] (m2) at (1.35, -1.55) {....  string arena};
\node[map] (m3) at (1.35, -2.05) {0x1B000 exc stack};
\node[map] (m4) at (1.35, -2.55) {0x1C000 frame stack};

\node[grp, draw=memborder, inner sep=5pt,
  fit=(grant)(mbox)(uart)(m0)(m1)(m2)(m3)(m4),
  label={[ttl,text=memborder]above:\textbf{Interconnect}}] (mid) {};

% ===================== EXCORE =====================
\node[ex, minimum width=3.2cm] (cpu) at (5.4, 7.15)
  {\textbf{RV32 CPU}\\{\scriptsize multicycle}};
\node[ex, minimum width=3.2cm] (mmio) at (5.4, 4.0)
  {MMIO bridge\\{\scriptsize mb $\cdot$ slot $\cdot$ TX}};
\node[ex, minimum width=3.2cm] (fw) at (5.4, 1.7)
  {Firmware\\{\scriptsize grow/rehash/print}};

\node[grp, draw=exborder, inner sep=5pt,
  fit=(cpu)(mmio)(fw),
  label={[ttl,text=exborder]above:\textbf{ExCore}}] (exg) {};

\draw[arr] (cpu) -- (cpu |- 0,8.55);
\draw[arr] (cpu) -- (cpu |- 0,5.55);
\draw[arr] (cpu) -- (fimem);
\draw[bus] (cpu) -- (mmio);
\draw[arr] (mmio) -- (mmio |- 0,2.85);
\draw[arr] (mmio) -- (mmio |- 0,5.55);
\draw[arr] (fw) -- (fimem);

\end{tikzpicture}%
}

\caption{Physical SoC view: memories on the I-bus and D-bus, PyCore datapath and
language engines as peer hardware blocks, ExCore as an RV32 CPU behind an MMIO
bridge, and a trap mailbox on a dedicated bus.
The DMEM map lists the principal shared address regions.
No control-pipeline stages are shown.}
\label{fig:block}
\end{figure}
\clearpage
\clearpage
