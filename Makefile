...
pycore-container-dict-lookup:
	...
	$(VERILATOR) -sv --binary --timing \
		+incdir+pycore/rtl \
		--top-module tb_container \
		-GPROG_HEX=\"pycore/programs/dict_build_lookup.hex\" \
		-GEXPECTED_TAG=4\'b0001 \
		"-GEXPECTED_VALUE=128\'d42" \
		--Mdir $(BUILD_DIR)/pycore_container_dict_lookup \
		-Wall -Wno-fatal \
		$(PYCORE_RTL_SRCS) pycore/tb/tb_container.sv
	./$(BUILD_DIR)/pycore_container_dict_lookup/Vtb_container
...
pycore-container-dict-store:
	...
	$(VERILATOR) -sv --binary --timing \
		+incdir+pycore/rtl \
		--top-module tb_container \
		-GPROG_HEX=\"pycore/programs/dict_store_subscr.hex\" \
		-GEXPECTED_TAG=4\'b0001 \
		"-GEXPECTED_VALUE=128\'d99" \
		--Mdir $(BUILD_DIR)/pycore_container_dict_store \
		-Wall -Wno-fatal \
		$(PYCORE_RTL_SRCS) pycore/tb/tb_container.sv
	./$(BUILD_DIR)/pycore_container_dict_store/Vtb_container

pycore-container: pycore-container-build-index pycore-container-store-subscr pycore-container-dict-lookup pycore-container-dict-store
