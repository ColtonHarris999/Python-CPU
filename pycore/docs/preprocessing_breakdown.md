...
**Scope note:** the committed `preprocess.py` is **single-function**. It
compiles one named entry function into an imem hex image. The
`multifn_*.hex` fixtures under `pycore/programs/` are **hand-assembled** with
the interim `CALL arg = {argc[15:0], slot[15:0]}` encoding and cannot currently
be regenerated by tooling. Restoring multi-function preprocessing lands with
the `LOAD_GLOBAL` / real `CALL` PR, not this one.
...
4. **Constant encoding**: for each `LOAD_CONST`, eagerly encodes `co_consts[N]`
   ...
   `EmittedInstruction`. Tuple/list/dict/set/frozenset constants raise
   `ValueError` (need the static heap image builder). `None` → `TAG_NONE`.
   Long strings are **interned** by `StringHeapBuilder` (identical payloads
   share one address) so dict key descriptor equality works.
...
Companion tool `pycore/tools/heap_image.py` builds static dmem images of
dicts/tuples/lists (scalars and string descriptors) for `DMEM_HEX` preload;
it mirrors `pycore_dict_key_hash` and reports `HEAP_INIT_PTR`.
