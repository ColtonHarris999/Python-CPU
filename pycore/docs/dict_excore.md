...
obj+16 : { version[63:0], order_len[63:0] }   // mutation counter + order length
...
The handle address is stable across grows. The hash table is used for
lookup/store/delete; the order sidecar is used for insertion-order iteration
(`FOR_ITER`). New-key insert appends to the order list and bumps `version`;
delete removes from the order list and bumps `version`; value-only overwrite
does neither.
...
Code 12 is `LIST_DELETE` (list shift-down). Codes 13–14 are `SET_GROW` /
`SET_UPDATE`. See `pycore/docs/set_excore.md`.
