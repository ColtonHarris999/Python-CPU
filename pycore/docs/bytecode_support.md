...
| `CHECK_EXC_MATCH`                       | `except T:` type test.                                                                      | Identity, then `tp_base` walk (depth 8). `except (A, B):` one-level tuples (length ≤ 8); nested tuples → `TYPE`. `[exc,type]→[exc,bool]`. Layer D: `img_try_stopiteration*`, `img_try_exception`, `img_try_tuple_match`, `img_class_myerror*`, `img_class_levendist_error`. |
...
| `CHECK_EXC_MATCH`        | `execute`      | landed-T1    | identity + `tp_base` walk + one-level tuples; T10 user subclasses reuse the same walk |
