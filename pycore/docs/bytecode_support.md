...
The analyzer (`pycore/tools/btanalyze`) reads that file. To check whether a
...
| `BINARY_SLICE`                          | Slice read `x[a:b]` (stack `subject, start, stop`).                                          | Strings (`CONT_SLICE_STR`) and LIST/TUPLE (`CONT_SLICE_SEQ`). String bounds are **character** indices; one UTF-8 walk resolves them to byte offsets (O(stop)). LIST/TUPLE copies the `[start:stop]` window into a newly allocated sequence (`capacity == length`). Out-of-range bounds **clamp** like CPython, `stop <= start` is empty, omitted bounds arrive as `None`. Negative bounds trap `PY_TRAP_TYPE` (deviation 3). Other subjects (INT/DICT/…) trap `PY_TRAP_TYPE`. All-literal unit-step slices are rewritten from slice-const + `NB_SUBSCR` at image build. Layer D: `img_slice_str*`, `img_slice_list*`, `img_slice_tuple`, `img_slice_str_const`, `img_slice_damerau`. |
...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BINARY_OP`            | Performs binary arithmetic/bitwise operation selected by `oparg`.                                                    | Arithmetic/bitwise opargs use the existing ALU path; `NB_SUBSCR` (oparg 26) routes to `S_CONTAINER`. `NB_INPLACE_ADD` (13) with a LIST lhs routes to LIST_EXTEND semantics and supports LIST/TUPLE rhs, including the existing excore grow path. `NB_MULTIPLY` / `NB_INPLACE_MULTIPLY` (5/18) with a LIST/TUPLE and an INT/BOOL (either side) allocate a new sequence (`CONT_SEQ_REPEAT`); count ≤ 0 yields empty; overflow / OOM → `MEM_FAULT`. Inplace `lst *= n` allocates a new list (does not mutate aliases). `NB_ADD` (0) with LIST+LIST or TUPLE+TUPLE allocates a new sequence (`CONT_SEQ_CONCAT`). Mixed `list+tuple` still `TYPE`. `STR * INT` still `TYPE`. Layer D: `img_list_repeat`, `img_list_repeat_jaro`, `img_list_repeat_type_trap`, `img_list_concat`, `img_list_concat_damerau`, `img_list_concat_type_trap`.                                                                                                                                                                                                                                                         |
...
| `LIST_EXTEND`          | Extends the list `oparg` slots below TOS with the iterable at TOS; pops only the iterable.                           | Empty LIST/TUPLE source is a no-op pop on pycore. Non-empty always raises `PY_TRAP_LIST_EXTEND` (trap 10); with `EXCORE_EN=1` the excore copies in place when `need <= cap`, else grows-to-fit, then `COMPLETED` pop 1. Unsupported iterable tags → `PY_TRAP_TYPE`. Emitted by compile() for list-display unpack (`[1,2,*x]`, `[*a,*b]`); `list.extend` / `list.append` method calls now resolve via the native `LOAD_ATTR` table and reuse this opcode from firmware. LIST `NB_INPLACE_ADD` (`+=`) also routes here. Fixtures: `list_extend_empty` / fatal (single-core), `extend_*` + `img_list_extend` / `img_list_methods` (two-core). |
...
2. **Missing dict key →** `PY_TRAP_MEM_FAULT`**.** `KeyError` is seeded
  (Wave A) but Track 6 has not converted this site yet. Absent keys still
...
11. **Unbound local →** `PY_TRAP_MEM_FAULT`**.** `UnboundLocalError` is seeded
  (Wave A) but Track 6 has not converted this site. A second `del`
...
14. **Slice bounds are unsigned and clamp; negatives trap.** `BINARY_SLICE`
  clamps out-of-range bounds exactly as CPython does, but a negative bound
   ...
   rule as deviation 3 for indices). So `s[1:99]` / `xs[1:99]` work and
   `s[1:-1]` / `xs[1:-1]` trap.
   Coverage: `img_slice_str_clamp`, `img_slice_str_neg_trap`,
   `img_slice_list_clamp`, `img_slice_list_neg_trap`.
...
16. **Exceptions propagate across ordinary Python frames.** On an exception-
  table miss, `RAISE_VARARGS` preserves the existing `OBK_EXCEPTION`, pops the
   ...
   `OBK_EXCEPTION.args` is empty for bare type raises and a one-element tuple
   for one-argument exception construction. `LOAD_ATTR` name `"args"` on an
   `OBK_EXCEPTION` returns that tuple. Coverage: `img_try_exc_types`,
   `img_try_exc_cross_frame_fatal`, `img_try_callee_unhandled`,
   `img_try_syntaxerror_msg`, `img_exc_args`.
...
| ------------------------ | -------------- | ------------ | ---- |
| `RAISE_VARARGS`          | `partial`      | T4 (oparg 2) | oparg 0 bare re-raise and oparg 1 type-vs-instance landed; no-active bare raise is fatal |
...
| `slice` **step ≠ 1**  | All-literal `s[a:b:n]` with `n` not 1.    | `BINARY_SLICE` has no step; the image compiler rewrites unit-step slice constants (`s[1:]`, `s[:]`, `s[1:3]`) to `BINARY_SLICE`. |
