...
| `COMPARE_OP`                            | Compares TOS-2 and TOS-1 using the packed CPython 3.14 `oparg`, replacing both with `BOOL`. | Selectors `0..5`; native `INT`/`BOOL`/`FLOAT` plus same-tag `SHORT_STR` equality **and** lexicographic ordering; same-tag `LONG_STR` equality of identical handles *and* of distinct objects with matching meta (`SA_CMP`); LONG_STR ordering via `SA_CMP`. Other tags → `TYPE`. Dict/set probes use the same three-tier STRACC compare so runtime concats are valid keys. Layer D: `img_compare_op`, `img_str_eq`, `img_str_eq_runtime_long`, `img_str_lt`, `img_sorted_str`. |
...
4. **Runtime strings as dict keys work.** LONG_STR dict/set equality is the
   ...
   `d[a + b]` hits (`img_str_dict_key_runtime`). `COMPARE_OP ==` / ordering
   on two distinct LONG objects with matching meta also uses `SA_CMP`
   (`img_str_eq_runtime_long`). Interned constants that share a handle still
   compare equal on the identity path (`img_str_eq`).
