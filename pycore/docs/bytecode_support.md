...
| `MAKE_FUNCTION` | Builds a function object from a code object. | Interim model: function is the `CODE_OBJECT` handle itself. Defaults are folded at image-build time into `co_defaults` (field 4); closures/annotations still rejected. |
| `CALL` | Invokes a callable with positional arguments. | Free-function (`NULL` sentinel) and method form (non-`NULL` self); `OBK_BOUND_METHOD` unwrap; `OBK_TYPE` instantiation (`__init__` + `ret_discard_push_self`). Argcount range uses `co_defaults`. Images: `img_method_*`, `img_ctor_*`, `img_default_*`. |
