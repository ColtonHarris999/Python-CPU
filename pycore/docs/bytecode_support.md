...
semantic ceilings that remain even when JSON says `execute` (e.g. mixed-tag
string ordering still TYPE-traps; STRACC MAP/CLASSIFY above U+00FF is a
...
| `COMPARE_OP`                            | Compares TOS-2 and TOS-1 using the packed CPython 3.14 `oparg`, replacing both with `BOOL`. | Selectors `0..5`; native `INT`/`BOOL`/`FLOAT` plus same-tag `SHORT_STR` equality **and** lexicographic ordering; same-tag `LONG_STR` equality (identical handles, or distinct equal-meta objects via STRACC `SA_CMP`) and `LONG_STR` ordering (also `SA_CMP`). Mixed SHORT/LONG `==`/`!=` is false. Other tags → `TYPE`. Dict/set probes use the three-tier STRACC compare (`SA_CMP`) so runtime concats are valid keys. Layer D: `img_compare_op`, `img_str_eq`, `img_str_lt`, `img_sorted_str`. |
...
4. **Runtime strings as dict keys work.** LONG_STR dict/set equality is the
   ...
   `d[a + b]` hits (`img_str_dict_key_runtime`). `COMPARE_OP ==` / `!=` on
   distinct LONG objects with matching `(hash, nbytes, nchars, kind)` also
   issues `SA_CMP` (`stracc_cmp` in `pycore_core.sv`). Interned constants
   that share a handle compare equal at tier 1 (`img_str_eq`).
