...
| ---------------------- | -------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BINARY_OP`            | Performs binary arithmetic/bitwise operation selected by `oparg`.                                                    | Arithmetic/bitwise opargs use the existing ALU path; `NB_SUBSCR` (oparg 26) routes to `S_CONTAINER`. `NB_INPLACE_ADD` (13) with a LIST lhs routes to LIST_EXTEND semantics and supports LIST/TUPLE rhs, including the existing excore grow path. `NB_MULTIPLY` / `NB_INPLACE_MULTIPLY` (5/18) with a LIST/TUPLE and an INT/BOOL (either side) allocate a new sequence (`CONT_SEQ_REPEAT`); count ≤ 0 yields empty; overflow / OOM → `MEM_FAULT`. Inplace `lst *= n` allocates a new list (does not mutate aliases). `STR * INT` still `TYPE`. Layer D: `img_list_repeat`, `img_list_repeat_jaro`, `img_list_repeat_type_trap`.                                                                                                                                                                                                                                                         |
