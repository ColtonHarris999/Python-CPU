...
Writes ``planning/bytecode_compile_progress.md``. Re-run after ISA or
vendor updates:
...
DEFAULT_OUT = ROOT / "planning" / "bytecode_compile_progress.md"
...
EXTRA_NOTES = {
    ...
    "LOAD_ATTR": (
        "INT/BOOL/STR/CODE_OBJECT/EXCEPTION/TYPE. LIST still traps "
        "(no list.append). Plan 1 P6.3.2."
    ),
    "LOAD_BUILD_CLASS": (
...
def render_markdown(report: MixReport) -> str:
    ...
    a("7. **Fully-supported count is necessary but not sufficient.** `LOAD_ATTR`")
    a("   still traps on LIST, so `list.append` in the vendor tree would trap")
    a("   even though `LOAD_ATTR` itself is `execute`.")
    a("")
    a("Plan 3 therefore does **not** try to execute this opcode mix. The")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_OUT,
        help="markdown path (default: planning/bytecode_compile_progress.md)",
    )
