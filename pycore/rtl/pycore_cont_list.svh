...
// Included inside pycore_core's unique case (container_op_r). Do not compile alone.
                        ...
                        CONT_SEQ_REPEAT: begin
                            // BINARY_OP multiply: LIST/TUPLE * INT/BOOL
                            // (or commuted). Allocates a new sequence.
                            // Negative / zero count → empty result.
                            unique case (container_phase_r)

                                CP_INIT: begin
                                    if (!pycore_is_seq_repeat(
                                            cont_rs1_tag, cont_rs1_val,
                                            cont_rs2_tag, cont_rs2_val)) begin
                                        container_type_trap_r <= 1'b1;
                                    end else begin
                                        container_src_is_tuple_r <=
                                            (cont_repeat_src_tag == PY_TAG_TUPLE);
                                        container_src_base_r     <= 32'd0;
                                        container_tomb_idx_r     <= 32'd0;
                                        if (cont_repeat_src_tag == PY_TAG_TUPLE) begin
                                            if ((cont_repeat_tuple_size[63:32] !=
                                                    32'b0)) begin
                                                container_mem_fault_r <= 1'b1;
                                            end else begin
                                                container_src_len_r  <=
                                                    cont_repeat_tuple_size[31:0];
                                                container_src_buf_r  <=
                                                    cont_repeat_src_addr;
                                                container_phase_r    <= CP_SRC_HDR;
                                            end
                                        end else begin
                                            container_base_r         <=
                                ...
                                CP_HDR: begin
                                    if (!container_dmem_pending_r) begin
                                        if (cont_hdr_len[63:32] != 32'b0) begin
                                            container_mem_fault_r <= 1'b1;
                                        end else if (cont_hdr_len == 64'd0) begin
                                            container_src_len_r <= 32'd0;
                                ...
                                // Compute dest_len, allocate, then copy.
                                CP_SRC_HDR: begin
                                    if (cont_repeat_count <= 64'sd0 ||
                                        container_src_len_r == 32'd0) begin
                                        container_src_slots_r <= 32'd0;
                                    ...
                                    end else if ((cont_repeat_count >
                                            64'sd2147483647) ||
                                        (container_src_len_r >
                                            (32'd2147483647 /
                                             cont_repeat_count[31:0]))) begin
                                        container_mem_fault_r <= 1'b1;
                                    end else begin
                                        container_src_slots_r <=
                                            container_src_len_r *
                                            cont_repeat_count[31:0];
                                        container_phase_r <= CP_DICT_META;
                                    end
                                end
                                ...
                                CP_DICT_META_FINAL: begin
                                    if (!container_dmem_pending_r) begin
                                        if (container_src_slots_r == 32'd0) begin
                                            container_wb_we_r   <= 1'b1;
                                            container_wb_addr_r <=
                                                RF_AW'(tos_r - RF_AW'(2));
                                            container_wb_data_r <=
                                ...
                                CP_COPY_TAG_WB: begin
                                    if (!container_dmem_pending_r) begin
                                        if (container_src_base_r + 32'd1 <
                                                container_src_slots_r) begin
                                            if (container_tomb_idx_r + 32'd1 ==
                                                    container_src_len_r)
                                                container_tomb_idx_r <= 32'd0;
                                            else
                                                container_tomb_idx_r <=
                        ...
                        CONT_SEQ_CONCAT: begin
                            // BINARY_OP add: LIST+LIST or TUPLE+TUPLE.
                            // Allocates a new sequence (lhs then rhs). Mixed
                            // kinds never reach this arm (ALU TYPE-traps).
                            unique case (container_phase_r)

                                CP_INIT: begin
                                    if (!pycore_is_seq_concat(
                                            cont_rs1_tag, cont_rs1_val,
                                            cont_rs2_tag, cont_rs2_val)) begin
                                        container_type_trap_r <= 1'b1;
                                    end else begin
                                        ...
                                        if (cont_rs1_tag == PY_TAG_TUPLE) begin
                                            if ((cont_tuple_size[63:32] !=
                                                    32'b0) ||
                                                (cont_tuple_size_rs2[63:32] !=
                                                    32'b0)) begin
                                                container_mem_fault_r <= 1'b1;
                                            end else begin
                                                container_src_len_r  <=
                                ...
                                CP_DICT_HASH: begin
                                    if (!container_dmem_pending_r) begin
                                        if (cont_hdr_len[63:32] != 32'b0) begin
                                            ...
                                            container_rhs_len_r <= 32'd0;
                                            container_rhs_buf_r <= 32'd0;
                                            container_phase_r   <= CP_SRC_HDR;
                                        end else begin
                                            container_rhs_len_r <=
                                                cont_hdr_len[31:0];
                                ...
                                CP_DICT_PROBE: begin
                                    if (!container_dmem_pending_r) begin
                                        container_rhs_buf_r <= cont_obitem_buf;
                                        container_phase_r   <= CP_SRC_HDR;
                                    end
                                end

                                CP_SRC_HDR: begin
                                    if ((container_src_len_r >
                                            (32'hFFFF_FFFF -
                                             container_rhs_len_r))) begin
                                        container_mem_fault_r <= 1'b1;
                                    end else begin
                                        container_src_slots_r <=
                                            container_src_len_r +
                                ...
                                CP_DICT_META: begin
                                    if ((container_src_slots_r == 32'd0) &&
                                        container_src_is_tuple_r) begin
                                        container_wb_we_r   <= 1'b1;
                                        container_wb_addr_r <=
                                            RF_AW'(tos_r - RF_AW'(2));
                                        container_wb_data_r <=
                        ...
                        CONT_SLICE_SEQ: begin
                            // BINARY_SLICE on LIST/TUPLE. Stack is
                            // [subject, start, stop]; result replaces subject
                            // and start/stop are popped (net -2).
                            unique case (container_phase_r)
                                ...
                                CP_HDR: begin
                                    if (!container_dmem_pending_r) begin
                                        if (cont_hdr_len[63:32] != 32'b0) begin
                                            container_mem_fault_r <= 1'b1;
                                        end else if (cont_hdr_len == 64'd0)
                                        begin
                                            container_src_len_r <= 32'd0;
                                            container_src_buf_r <= 32'd0;
                                            container_phase_r   <= CP_VAL;
                                ...
                                CP_VAL: begin
                                    ...
                                    end else if (!container_dmem_pending_r)
                                    begin
                                        container_val_r <= container_rd_data_r;
                                        if (container_src_is_tuple_r) begin
                                            container_dmem_addr_r <=
                                ...
                                CP_SRC_HDR: begin
                                    // Clamp unsigned start/stop to src_len.
                                    // None-stop is 0xFFFFFFFF → clamps to len.
                                    if (container_src_idx_r >
                                            container_src_len_r)
                                        container_src_idx_r <=
                                            container_src_len_r;
                                    if (container_slice_stop_r >
                                            container_src_len_r)
                                        container_slice_stop_r <=
                                            container_src_len_r;
                                    container_phase_r <= CP_DICT_META;
                                ...
                                // dest_len uses the clamped registers written
                                // last cycle so the multiply/sub is stable.
                                CP_DICT_META: begin
                                    if (container_slice_stop_r <=
                                            container_src_idx_r) begin
                                        container_src_slots_r <= 32'd0;
                                        container_tomb_idx_r  <=
                                            container_src_idx_r;
                                        container_src_len_r   <=
