...
module pycore_core #(
        ...
        if (state_r == S_CONTAINER && container_stracc_issue) begin
            ...
            if (container_phase_r == CP_DICT_CHK_VAL) begin
                ...
                stracc_cmd_a = pycore_make_entry(
                    cont_order_scan_needle_tag, container_val_r);
                stracc_cmd_b = pycore_make_entry(
    ...
    logic [3:0] cont_order_scan_needle_tag;
    logic       cont_dict_order_need_cmp;
    logic       cont_dict_order_key_match;
    ...
    // DELETE_DICT latches the needle tag in container_order_key_tag_r;
    // DELETE_ATTR keeps it in container_tag_r (interned co_names). The
    // order buffer's (tag, val) pair is (rd_data[3:0], order_shift_val).
    assign cont_order_scan_needle_tag =
        (container_op_r == CONT_DELETE_DICT) ? container_order_key_tag_r
                                             : container_tag_r;
    assign cont_dict_order_need_cmp =
    ...
    assign cont_dict_order_key_match = pycore_dict_key_rich_eq(
        cont_order_scan_needle_tag, container_val_r,
        container_rd_data_r[3:0], container_order_shift_val_r) ||
        (container_stracc_done_r && container_stracc_eq_r &&
         (container_phase_r == CP_DICT_ORDER_SCAN_TAG));
    // Two sequential SA_CMP episodes can occur in one DELETE: table probe
    // at CHK_VAL, then order-buffer scan at ORDER_SCAN_TAG. done/eq are
    // single-shot and must drop when leaving each of those phases.
